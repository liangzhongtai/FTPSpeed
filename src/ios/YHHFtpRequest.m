...
- (void)setupDataSocket {
    NSLog(@"----------被动模式返回的%@, %d", _dataHost, _dataPort);
    if (![_dataHost isEqualToString:self.serverIP]) {
        _dataHost = self.serverIP;
    }
    NSInputStream *input;
    NSOutputStream *output;
    
    if (_ctype >= OperateUpload) {
        NSLog(@"-------------------上传");
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode {
    ...
    if (eventCode == NSStreamEventHasBytesAvailable) {
        ...
        _fsize += wrlen;
        NSLog(@"读到了数据_write: %ld, read: %ld", wrlen, relen);
        if (_ctype == OperateUpload) {
